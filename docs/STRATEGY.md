...
4. Publicar generación de SDKs adicionales, changelog y SLOs medidos; luego ejecutar homologación y certificaciones.
...
## Benchmark aplicado — book transfers y estados de cuenta (30/08/2026)

[Pismo](https://developers.pismo.io/pismo-docs/docs/single-ledger-entries-and-book-transfers) documenta un book transfer como débito y crédito ejecutados atómicamente sobre el ledger central, con tracking id, validaciones de cuenta/ledger y cancelación mediante postings opuestos. Su guía de Payments separa cash-in, cash-out e internal transfer entre cuentas de plataforma. [Pomelo P2P](https://developers.pomelo.la/api-reference/balance-authorization/movements/authorize-p2p-transaction) refuerza el contrato de origen, destino e idempotencia. [BIND/bindX](https://developers.bindx.com/) expone consulta de movimientos, fechas, importes, contrapartes y saldos como base de conciliación, además de transferencias 7x24 por riel bancario. El patrón competitivo verificable es atomicidad + validación + tracking + historial; una transferencia externa 7x24 no se puede inferir de un movimiento interno.

Cimbra implementa el rail interno propio con una decisión deliberadamente más restrictiva: no existe `force_post` ni bypass de estado o saldo. `book_transfers` vincula dos cuentas activas del mismo tenant y moneda con una transacción canónica; el débito del origen y crédito del destino comparten un journal SQL balanceado. La disponibilidad resta holds activos, el riesgo puede reservar sólo el origen, `transfer.create` protege maker/checker, las reversas agregan postings compensatorios y el statement por cuenta calcula saldos de apertura/cierre sobre entradas inmutables. API v1, SDK, webhooks y consola usan el mismo recurso; Viewer sólo consulta y Owner/Admin/Operator operan según `finance.write`.
...
Cimbra implementa el dominio propio sobre el financial core: emite CVU sandbox con prefijo `000` y código PSP `9999` (no asignado por Coelsa), alias único por tenant con asignación o cambio sobre un CVU existente (unicidad del tenant, un cambio real cada 24 h; la emisión o la primera asignación no arrancan esa ventana), eliminación del CVU sin tocar cuenta ni saldo (se puede volver a emitir el mismo CVU sandbox), preview de titular, crédito interno via postings atómicos o cash-out a settlement, débito sólo entre cuentas Cimbra, QR dinámico `cimbra:qr:v1` (un pago, con vencimiento), QR estático `cimbra:qr:static:v1` (reutilizable, sin vencimiento; uno activo por cuenta si `owner=account`, y uno por `collection_till` si `owner=till`), orden de venta Cimbra sobre ese QR estático (monto cerrado, una pendiente por QR; crear otra reemplaza la pendiente) y QR de deuda `cimbra:qr:debt:v1` (recurso primario deuda, monto cerrado, un solo pago, exige CVU sandbox activo) y returns compensatorios. API v1, SDK, webhooks y consola por rol comparten el contrato. No hay directorio nacional, DEBIN externo ni QR interoperable. No hay conexión a BIND ni overlay EMVCo/Transferencias 3.0 de caja.
...
Cimbra deja el puerto preparado y fail-closed. `official_rail_connections` persiste `unwired | negotiating | contracted | certified | live` con evidencia de sponsor (razón social, CUIT, contrato, cuenta safeguarding y due diligence), sin secretos. Cada producto del catálogo declara los rieles que necesita. `liveReady` exige hostname de producción, producto en Go Live, esos rieles en `live` y un adaptador Cimbra registrado en `OFFICIAL_RAIL_ADAPTERS` (fondos) o el adaptador documental del `sponsor_bank` cuando la evidencia está completa. El registro de adaptadores de fondos está vacío: `dispatchOfficialRail` responde `rail_not_wired` o `rail_adapter_missing`. BIND Banco (entidad financiera) es candidato a banco patrocinante; BIND PSP / bindX siguen siendo benchmarks de producto, nunca el core ni un conector. El adaptador futuro de cámara vive fuera del ledger y no abre una transacción SQL durante la llamada de red.

El envelope comercial de USD 500 cubre Gate 1 (`capitalPlan` en live-readiness y `/investors`): consulta legal, dominio, entrevistas y correo. No autoriza AWS pago, membresía Coelsa, retainer de banco patrocinante, PCI, constitución SAS ni marcar Go Live. El trámite de inscripción PSP del BCRA es gratuito, pero exige persona jurídica argentina y, para PSPCP, banco patrocinante. El próximo cheque (pre-seed, monto no tasado) es el que puede financiar esa figura.

## Benchmark aplicado — servicios de dominio y control plane (02/09/2026)

Pismo, Pomelo y Dock publican cores cloud-native con perímetros separados para identidad, ledger, rieles, riesgo y entrega de eventos. El patrón verificable no es “un microservicio por archivo”: es propiedad de datos, contrato API, eventos y una compuerta explícita para extraer runtime. Extraer por moda, con un solo operador y USD 500, destruiría el capital y el aislamiento.

Cimbra declara 21 servicios de dominio en `app/lib/platform/service-catalog.ts`. Cada tabla y cada módulo que escribe datos pertenece a exactamente un servicio. `tests/service-boundaries.test.ts` rechaza escrituras cruzadas no declaradas. La deuda de extracción (ledger que asienta un cobro, identidad que crea el tenant) es visible. `GET /api/v1/services` y `/status` publican la topología. Ningún servicio corre `standalone`: extraer exige volumen, riesgo, disponibilidad o perímetro regulatorio, y autorización expresa de gasto. BIND, Dock, tapi, Pismo, Pomelo y Wibond no son destinos de extracción ni de contract tests.

El control plane (`/ops`) se separa del plano de datos del tenant como en las plataformas comparables: operadores de Cimbra provisionados por lista, no por rol owner del sandbox. Soporte es un servicio propio con caso, historial y webhook, no un mailto.

## Benchmark aplicado — cobranzas Argentina (01/09/2026)

[BIND PSP Cobro](https://psp.bind.com.ar/developers/cobro) publica botón de pago, links asociados a una deuda, recaudación por transferencia/CVU, QR y POS. [Crear un link 2.0](https://psp.bind.com.ar/developers/apis/boton20-crearlinkdepago) expone un detalle de checkout informativo; [consultar un link](https://psp.bind.com.ar/developers/apis/boton20-consultarlink) lista los pagos; [devolver un link 2.0](https://psp.bind.com.ar/developers/apis/boton20-devolverlinkdepago) admite el total o un parcial. El patrón competitivo verificable es intención de cobro + detalle + medios + acreditación + consulta de créditos + devolución parcial + devolución; una adquirencia de red no se infiere de un link sandbox.

Cimbra implementa el dominio propio: `payment_links` contra una cuenta ARS argentina, payload `cimbra:link:v1`, cobro interno entre cuentas del tenant o inbound ledger, y vínculo opcional a una deuda QR abierta (`cimbra_qr`) o a un `collection_till` activo (`cimbra_cvu`) para mostrar el payload del QR o el CVU del punto. Los `items` van en la raíz del create (máx. 20) y son detalle de checkout: no controlan el monto ni sustituyen un carrito de adquirente. `cimbra_qr` e `internal` cierran el link al monto cerrado; `cimbra_cvu` acredita el till en parciales, varios créditos o por más del restante y expone `checkoutUrl` `/pay/{id}` sin formulario de tarjeta. GET, list y pay embeben `credits`, `refunds`, `partiallyCollected` y `partiallyRefunded`. POST refund acepta `amount` y `creditId` opcionales: vacío devuelve lo cobrado pendiente; un importe parcial deja asientos compensatorios. Un link CVU puede reabrirse si el neto cobrado queda por debajo del monto; internal, QR e inbound permanecen cerrados tras una devolución parcial. Pagar el QR de la deuda también liquida el link asociado. `collection_tills` sigue siendo un punto de recaudación con CVU sandbox propio (no un segundo CVU de la cuenta), alias de tenant, QR estático propio (`issueStaticQr` o `POST .../static-qr`, payload inmutable, `owner=till`, no consume el QR estático de la cuenta), `closedAmountOnly` para exigir orden de venta, `presence` presente/no presente (no es POS), acreditación inbound o transferencia interna atribuida al till, y deshabilitación irreversible. Un inbound suelto al till no imputa el link. `successUrl`, `errorUrl` y `configuracionCheckout` se rechazan. Tarjeta, POS, Tap to Phone, checkout PCI y QR interoperable responden `422` con código explícito. API v1, SDK, webhooks y consola por rol comparten el contrato.

La disponibilidad `sandbox` no es checkout PCI, sucursal/caja BIND, liquidación a un adquirente, conciliación de marca ni crédito Coelsa. Esas capas entran con licencia o sponsor, certificación EMV/PCI y riel directo. BIND, Dock, tapi, Pismo, Pomelo y Wibond siguen siendo benchmarks, no conectores.

## Benchmark aplicado — ECHEQ Argentina (01/09/2026)

El [BCRA](https://www.bcra.gob.ar/emitir-un-cheque-y-usar-echeq/) describe el ciclo público: emitir contra cuenta corriente con CUIT del beneficiario, aceptar, endosar si es a la orden, depositar, anular antes de la aceptación y devolver si ya fue aceptado. BIND y otras entidades publican APIs bancarias sobre ese riel Coelsa; el patrón competitivo verificable es instrumento + tenedor + depósito + rechazo, no copiar sus contratos.

Cimbra implementa el dominio propio: `echeqs` contra una cuenta ARS argentina, payload `cimbra:echeq:v1`, CUIT con dígito AFIP, aceptación y endoso tenant-scoped, depósito inmediato en otra cuenta Cimbra, rechazo por fondos insuficientes y devolución previa al depósito. Descuento, custodia, USD y compensación Coelsa responden `422`. API v1, SDK, webhooks y consola por rol comparten el contrato.

La disponibilidad `sandbox` no habilita un ECHEQ real. Emisión productva exige cuenta corriente, membresía o sponsor y riel directo con Coelsa. BIND, Dock, tapi, Pismo, Pomelo y Wibond siguen siendo benchmarks, no conectores.
...
Cimbra adopta el estándar de claridad como propiedad propia: `/developers` publica la única base URL activa, diferencia el sandbox (BIND APIBANK, Pismo `sandbox.pismolabs.io`, Pomelo `sandbox.api.pomelo.la`) de production sin hostname, genera la referencia de 194 operaciones desde OpenAPI, consume scopes y eventos canónicos, ofrece quickstarts exactos, errores estructurados, rate limits, paginación, modelo monetario, contrato de firma y calendario de reintentos. El SDK TypeScript se distribuye como artefacto real con checksum mientras no exista publicación npm. La colección Postman se genera desde el mismo OpenAPI y se descarga desde `/postman/cimbra-sandbox.postman_collection.json`; SDKs adicionales se muestran explícitamente como backlog. La consola interactiva contra endpoints mutantes se posterga hasta poder aislar credenciales y datos de forma segura. Dock, tapi y Wibond continúan como benchmarks de producto; no se atribuyen contratos técnicos específicos cuando la documentación pública consultada no alcanza para verificarlos.
