...
Cimbra implementa el dominio propio: `payment_links` contra una cuenta ARS argentina, payload `cimbra:link:v1`, cobro interno entre cuentas del tenant o inbound ledger, y vínculo opcional a una deuda QR abierta (`cimbra_qr`) o a un `collection_till` activo (`cimbra_cvu`) para mostrar el payload del QR o el CVU del punto. `cimbra_qr` e `internal` cierran el link al monto cerrado; `cimbra_cvu` acredita el till en parciales, varios créditos o por más del restante y expone `checkoutUrl` `/pay/{id}` sin formulario de tarjeta. Pagar el QR de la deuda también liquida el link asociado. `collection_tills` sigue siendo un punto de recaudación con CVU sandbox propio (no un segundo CVU de la cuenta), alias de tenant, acreditación inbound o transferencia interna atribuida al till, y deshabilitación irreversible. Un inbound suelto al till no imputa el link. Tarjeta, POS, Tap to Phone, checkout PCI y QR interoperable responden `422` con código explícito. API v1, SDK, webhooks y consola por rol comparten el contrato.
