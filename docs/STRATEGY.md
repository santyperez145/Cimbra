...
Cimbra implementa el dominio propio: `payment_links` contra una cuenta ARS argentina, payload `cimbra:link:v1`, cobro interno entre cuentas del tenant o inbound ledger, riesgo/holds y refunds compensatorios; y `collection_tills`, un punto de recaudación con CVU sandbox propio (no un segundo CVU de la cuenta), alias de tenant, acreditación inbound o transferencia interna atribuida al till, y deshabilitación irreversible. Tarjeta, POS, Tap to Phone y QR interoperable responden `422` con código explícito. API v1, SDK, webhooks y consola por rol comparten el contrato.
