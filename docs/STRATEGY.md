...
Cimbra implementa el dominio propio sobre el financial core: emite CVU sandbox con prefijo `000` y código PSP `9999` (no asignado por Coelsa), alias único por tenant con asignación o cambio sobre un CVU existente (unicidad del tenant, un cambio real cada 24 h; la emisión o la primera asignación no arrancan esa ventana), preview de titular, crédito interno via postings atómicos o cash-out a settlement, débito sólo entre cuentas Cimbra, QR con payload `cimbra:qr:v1` y returns compensatorios. API v1, SDK, webhooks y consola por rol comparten el contrato. No hay directorio nacional, DEBIN externo ni QR interoperable. No se implementan QR de caja/deuda ni baja de CVU hasta tener el riel homologado.
