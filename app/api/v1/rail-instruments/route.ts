...
import { normalizeIssueInstrumentInput } from '@/app/lib/platform/instant-payments-input';
import { InstantPaymentError, issueRailInstruments, listRailInstruments } from '@/db/instant-payments';
...
async function list(request: Request) {
  try {
    ...
    const rows = await listRailInstruments({ organizationId: principal.organizationId, limit, cursor: cursor ?? undefined });
...
async function create(request: Request) {
  try {
    ...
    const instrument = normalizeIssueInstrumentInput(await request.json().catch(() => null));
    if (!instrument) return NextResponse.json({ error: 'Datos de instrumento inválidos.', code: 'invalid_rail_instrument' }, { status: 400 });
    const result = await issueRailInstruments({ organizationId: principal.organizationId, actor: principal.user, idempotencyKey, instrument });
