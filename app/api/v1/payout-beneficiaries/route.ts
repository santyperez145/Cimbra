...
import { normalizePayoutBeneficiaryInput } from '@/app/lib/platform/payouts-input';
import { versionedApi } from '@/app/lib/platform/versioned-api';
import { createPayoutBeneficiary, listPayoutBeneficiaries } from '@/db/payouts';
...
async function list(request: Request) {
  try {
    ...
    return NextResponse.json({ data: await listPayoutBeneficiaries(principal.organizationId) },
...
async function create(request: Request) {
  try {
    ...
    const input = normalizePayoutBeneficiaryInput(await request.json().catch(() => null));
    if (!input) return NextResponse.json({ error: 'Beneficiario inválido.', code: 'invalid_payout_beneficiary' }, { status: 400 });
    const result = await createPayoutBeneficiary({ organizationId: principal.organizationId, actor: principal.user, idempotencyKey, ...input });
