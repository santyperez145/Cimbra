...
import { normalizeMandateInput } from '@/app/lib/platform/billers-input';
...
async function list(request: Request) {
  try {
    ...
    return NextResponse.json({ data: await listRecurringMandates(principal.organizationId) }, { headers: { 'Cache-Control': 'no-store', ...rateLimitHeaders(principal) } });
