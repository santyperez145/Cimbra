...
import { ApprovalError, requestSettlementExecutionApproval, requiresSettlementApproval } from '@/db/approvals';
  ...
  try {
    ...
    if (await requiresSettlementApproval(principal.organizationId)) {
      const result = await requestSettlementExecutionApproval({ organizationId: principal.organizationId,
        actorId: principal.user.userId, cycleId: id, idempotencyKey, executionMode: 'manual' });
      if (!result.replayed) scheduleWebhookDispatch(principal.organizationId);
      const completed = result.approval.status === 'executed';
      return NextResponse.json({ ok: true, requiresApproval: true, ...result }, {
        status: completed ? 200 : 202, headers: rateLimitHeaders(principal),
      });
    }
    ...
    if (error instanceof IdempotencyError || error instanceof SettlementError || error instanceof ApprovalError) {
