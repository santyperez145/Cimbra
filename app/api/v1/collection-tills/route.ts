...
import { normalizeCollectionTillInput } from '@/app/lib/platform/collections-input';
import { CollectionError, createCollectionTill, listCollectionTills } from '@/db/collections';
...
async function list(request: Request) {
  try {
    ...
    const rows = await listCollectionTills({ organizationId: principal.organizationId, limit, cursor: cursor ?? undefined });
...
async function create(request: Request) {
  try {
    ...
    const parsed = normalizeCollectionTillInput(await request.json().catch(() => null));
    if (!parsed) return NextResponse.json({ error: 'Datos de punto de recaudación inválidos.', code: 'invalid_collection_till' }, { status: 400 });
    const result = await createCollectionTill({
      organizationId: principal.organizationId, actor: principal.user, idempotencyKey, till: parsed,
    });
