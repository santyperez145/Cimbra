...
import { normalizeQrDebtInput } from '@/app/lib/platform/instant-payments-input';
import { InstantPaymentError, createQrDebt, listQrDebts } from '@/db/instant-payments';
...
async function list(request: Request) {
  try {
    ...
    const rows = await listQrDebts({ organizationId: principal.organizationId, limit, cursor: cursor ?? undefined });
...
async function create(request: Request) {
  try {
    ...
    const debt = normalizeQrDebtInput(await request.json().catch(() => null));
    if (!debt) return NextResponse.json({ error: 'Datos de deuda QR inválidos.', code: 'invalid_qr_debt' }, { status: 400 });
    const result = await createQrDebt({ organizationId: principal.organizationId, actor: principal.user, idempotencyKey, debt });
