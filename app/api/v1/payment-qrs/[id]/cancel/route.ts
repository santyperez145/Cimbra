...
import { InstantPaymentError, cancelPaymentQr } from '@/db/instant-payments';
...
async function create(request: Request, qrId: string) {
  try {
    ...
    const result = await cancelPaymentQr({
      organizationId: principal.organizationId, actor: principal.user, qrId, idempotencyKey,
    });
