...
import { normalizePaymentQrInput } from '@/app/lib/platform/instant-payments-input';
import { InstantPaymentError, createPaymentQr, listPaymentQrs } from '@/db/instant-payments';
...
async function list(request: Request) {
  try {
    ...
    const rows = await listPaymentQrs({ organizationId: principal.organizationId, limit, cursor: cursor ?? undefined });
...
async function create(request: Request) {
  try {
    ...
    const qr = normalizePaymentQrInput(await request.json().catch(() => null));
    if (!qr) return NextResponse.json({ error: 'Datos de QR inválidos.', code: 'invalid_payment_qr' }, { status: 400 });
    const result = await createPaymentQr({ organizationId: principal.organizationId, actor: principal.user, idempotencyKey, qr });
