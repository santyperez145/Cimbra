...
async function createEvaluation(request: Request) {
  try {
    ...
    if (!operationType || !currency || counterparty.length < 2 || !rawSignals) return NextResponse.json({ error: 'Evaluación de riesgo inválida.', code: 'invalid_risk_evaluation' }, { status: 400 });
    ...
    const evaluation = await evaluateAndPersistRisk({ organizationId: principal.organizationId, actor: principal.user,
      idempotencyKey: `evaluation:${idempotencyKey}`, operationType, amountMinor, currency, counterparty, signals });
