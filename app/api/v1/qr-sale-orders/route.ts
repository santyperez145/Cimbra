...
import { normalizeQrSaleOrderInput } from '@/app/lib/platform/instant-payments-input';
import { InstantPaymentError, createQrSaleOrder, listQrSaleOrders } from '@/db/instant-payments';
...
async function list(request: Request) {
  try {
    ...
    const rows = await listQrSaleOrders({ organizationId: principal.organizationId, limit, cursor: cursor ?? undefined });
...
async function create(request: Request) {
  try {
    ...
    const order = normalizeQrSaleOrderInput(await request.json().catch(() => null));
    if (!order) return NextResponse.json({ error: 'Datos de orden de venta inválidos.', code: 'invalid_sale_order' }, { status: 400 });
    const result = await createQrSaleOrder({ organizationId: principal.organizationId, actor: principal.user, idempotencyKey, order });
