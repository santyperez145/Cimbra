...
import { cancelEcheq, EcheqError } from '@/db/echeqs';
...
async function create(request: Request, echeqId: string) {
  try {
    ...
    const result = await cancelEcheq({
      organizationId: principal.organizationId, actor: principal.user, echeqId, idempotencyKey,
    });
