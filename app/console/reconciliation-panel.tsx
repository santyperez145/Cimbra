...
export default function ReconciliationPanel() {
  ...
  return <div className="module-view reconciliation-console">
    ...
    <div className="integration-grid reconciliation-grid"><article className="integration-card"><div className="card-head"><div><h2>Nuevo settlement</h2><p>Requiere conciliación cerrada y es sandbox</p></div><b>SCHEDULED</b></div><form className="integration-form" onSubmit={createSettlement}><label>Conciliación<select name="reconciliationRunId" required defaultValue=""><option value="" disabled>Seleccionar corrida</option>{runs.filter((run) => run.status === 'completed' && !settlements.some((cycle) => cycle.reconciliationRunId === run.id)).map((run) => <option key={run.id} value={run.id}>{run.name} · {run.currency}</option>)}</select></label><label>Nombre<input name="name" placeholder="Liquidación banco diaria" minLength={2} required /></label><label>Programar (opcional)<input name="scheduledFor" type="datetime-local" /></label><button disabled={busy}>Crear ciclo</button></form></article><article className="integration-card"><div className="card-head"><div><h2>Ciclos de settlement</h2><p>Confirmación operativa, sin fondos reales</p></div><b>{settlements.length}</b></div><div className="integration-list compact-list">{settlements.length === 0 ? <p>Sin ciclos creados.</p> : settlements.map((cycle) => <div key={cycle.id}><span><strong>{cycle.name}</strong><small>{cycle.rail} · {money(cycle.net, cycle.currency)} · diferencia {money(cycle.difference, cycle.currency)}{cycle.scheduledFor ? ` · ${new Date(cycle.scheduledFor).toLocaleString('es-AR')}` : ''}</small></span><b className={cycle.status}>{cycle.status}</b>{(cycle.status === 'ready' || cycle.status === 'scheduled' && Boolean(cycle.scheduledFor) && Date.parse(cycle.scheduledFor!) <= currentTime) && <button disabled={busy} onClick={() => void executeSettlement(cycle.id)}>Ejecutar</button>}</div>)}</div></article></div>
