...
export default function RiskPanel({ holds, busy: externalBusy, canManageRules, canResolve, onHold }: {
  ...
  return <div className="module-view risk-console">
    ...
    <div className="integration-grid risk-grid">
      {canManageRules ? <article className="integration-card"><div className="card-head"><div><h2>Nueva regla</h2><p>Configurable por tenant y evaluada por prioridad</p></div><b>NATIVE</b></div>
      ...
      <article className="integration-card"><div className="card-head"><div><h2>Políticas activas</h2><p>Sistema inmutable + reglas del tenant</p></div><b>{systemPolicies.length + rules.length}</b></div><div className="integration-list compact-list">
        ...
        {rules.map((rule) => <div key={rule.id}><span><strong>{rule.name}</strong><small>{rule.kind} · {rule.operationType} · +{rule.scoreDelta} · {rule.action}</small></span><b className={rule.status}>{rule.status}</b>{canManageRules && rule.status === 'active' && <button disabled={busy} onClick={() => void disableRule(rule.id)}>Deshabilitar</button>}</div>)}
      </div></article>
    </div>
