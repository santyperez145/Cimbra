...
const roleLabels: Record<Role, string> = { owner: 'Owner', admin: 'Admin', operator: 'Operator', viewer: 'Viewer' };
const roleDescriptions: Record<Role, string> = {
  owner: 'Gobierno total, accesos, seguridad y operación.',
  admin: 'Configura el tenant y gestiona operadores y viewers.',
  operator: 'Ejecuta y resuelve flujos operativos.',
  viewer: 'Consulta operación y evidencia sin mutaciones.',
};
...
export default function AccessPanel({ actorRole }: { actorRole: Extract<Role, 'owner' | 'admin'> }) {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/platform/access', { method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function changeRole(member: Member, role: Exclude<Role, 'owner'>) {
    ...
    const response = await fetch(`/api/platform/access/members/${member.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ role }) });
  ...
  async function removeMember(member: Member) {
    ...
    setBusy(true); setFeedback('');
    const response = await fetch(`/api/platform/access/members/${member.id}`, { method: 'DELETE' });
    const result = await response.json() as { error?: string };
  ...
  async function revoke(invitation: Invitation) {
    setBusy(true); setFeedback('');
    const response = await fetch(`/api/platform/access/invitations/${invitation.id}`, { method: 'DELETE' });
    const result = await response.json() as { error?: string };
  ...
  function manageable(member: Member) {
    return member.userId !== currentUserId && member.role !== 'owner' && (actorRole === 'owner' || member.role !== 'admin');
  }
