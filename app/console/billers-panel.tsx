...
export default function BillersPanel({ accounts, actorRole }: { accounts: DashboardData['accounts']; actorRole: OrganizationRole }) {
  ...
  return <div className="module-view billers-console">
    <div className="module-view-head"><div><p>BILL PAYMENTS · TOP-UPS · RECURRING</p><h1>Servicios y recargas</h1><span>Catálogo propio, deuda emitida, órdenes ledger-backed y mandatos con consentimiento. El control maker/checker vive en el alta del mandato, no en cada cargo del worker.</span></div><span className="module-health"><i /> Sandbox nativo</span></div>
    ...
    <div className="compliance-onboarding-grid">
      {canOperate && selected.status === 'active' && <article className="integration-card"><div className="card-head"><div><h2>Nuevo mandato</h2><p>Consentimiento declarado · alta con política recurring_mandate.create pasa por Aprobaciones</p></div><b>RECURRING</b></div><form className="integration-form" onSubmit={createMandate}><label>Cuenta<select name="accountId" required defaultValue=""><option value="" disabled>Seleccionar cuenta</option>{accounts.filter((account) => account.currency === selected.currency && account.status === 'active').map((account) => <option key={account.id} value={account.id}>{account.accountReference} · {account.currency}</option>)}</select></label><div className="integration-fields"><label>Suscriptor/destino<input name="subscriberReference" minLength={4} autoComplete="off" required /></label><label>Frecuencia<select name="frequency"><option value="monthly">Mensual</option><option value="weekly">Semanal</option></select></label></div>{selected.serviceType !== 'bill_payment' && <label>Monto por ejecución<input name="amount" type="number" min={selected.minAmount ?? 0.01} max={selected.maxAmount ?? 10000000} step="0.01" required /></label>}<div className="integration-fields"><label>Límite por ejecución<input name="amountLimit" type="number" min="0.01" step="0.01" required /></label><label>Próxima ejecución<input name="nextChargeAt" type="datetime-local" defaultValue={localDateInput()} required /></label></div><label>Referencia de consentimiento<input name="consentReference" minLength={3} maxLength={120} placeholder="CONSENT-2026-001" required /></label><button disabled={busy}>Crear mandato</button><small>Los cargos del worker no pasan por bill_payment.create: usan la exención auditada standing_mandate. En producción validá consentimiento y riel oficial.</small></form></article>}
