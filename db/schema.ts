...
export const approvalPolicies = pgTable('approval_policies', {
  ...
  check('approval_policies_action', sql`${table.actionType} IN ('settlement.execute', 'transfer.create', 'transfer.reverse', 'payment.create', 'payment.reverse', 'bill_payment.create', 'bill_payment.reverse', 'payout_batch.execute', 'risk.case.resolve', 'reconciliation.exception.resolve', 'dispute.resolve')`),
...
export const approvalRequests = pgTable('approval_requests', {
  ...
  check('approval_requests_action_resource', sql`(
    ...
    (${table.actionType} = 'transfer.reverse' AND ${table.resourceType} IN ('transfer', 'book_transfer')) OR
...
export const paymentQrs = pgTable('payment_qrs', {
  ...
  owner: text('owner').notNull().default('account'),
  ...
  uniqueIndex('idx_payment_qrs_account_active_static').on(table.accountId).where(sql`${table.kind} = 'static' AND ${table.status} = 'active' AND ${table.owner} = 'account'`),
  ...
  check('payment_qrs_owner', sql`${table.owner} IN ('account', 'till')`),
...
export const collectionTills = pgTable('collection_tills', {
  ...
  status: text('status').notNull().default('active'),
  presence: text('presence').notNull().default('not_present'),
  closedAmountOnly: integer('closed_amount_only').notNull().default(0),
  assignIdempotencyKey: text('assign_idempotency_key'),
  qrIdempotencyKey: text('qr_idempotency_key'),
  cancelIdempotencyKey: text('cancel_idempotency_key'),
  createdBy: text('created_by').notNull().references(() => users.id, { onDelete: 'restrict' }),
  createdAt: text('created_at').notNull(), updatedAt: text('updated_at').notNull(),
}, (table) => [
  ...
  uniqueIndex('idx_collection_tills_org_qr_idempotency').on(table.organizationId, table.qrIdempotencyKey).where(sql`${table.qrIdempotencyKey} IS NOT NULL`),
  ...
  check('collection_tills_presence', sql`${table.presence} IN ('present', 'not_present')`),
  check('collection_tills_closed_amount', sql`${table.closedAmountOnly} IN (0, 1)`),
]);
