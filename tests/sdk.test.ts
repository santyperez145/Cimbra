...
test('recurringMandates.resume tipa maker/checker opt-in', async () => {
  const client = new Cimbra({ baseUrl: 'https://api.test', apiKey: 'cim_test', fetch: async () =>
    Response.json({
      ok: true, requiresApproval: true, replayed: false, deduplicated: false,
      approval: { id: 'approval_mandate_resume_1', actionType: 'recurring_mandate.resume', resourceType: 'recurring_payment_mandate', resourceId: 'mandate_1', status: 'pending' },
    }, { status: 202 }) });
  const result = await client.recurringMandates.resume('mandate_1');
  assert.equal(result.data.requiresApproval, true);
...
test('echeqs.deposit tipa maker/checker opt-in', async () => {
  const client = new Cimbra({ baseUrl: 'https://api.test', apiKey: 'cim_test', fetch: async () =>
    Response.json({
      ok: true, requiresApproval: true, replayed: false, deduplicated: false,
      approval: { id: 'approval_echeq_1', actionType: 'echeq.deposit', resourceType: 'echeq', resourceId: 'echeq_1', status: 'pending' },
    }, { status: 202 }) });
  const result = await client.echeqs.deposit('echeq_1', { accountId: 'account_2', taxId: '30000075678' });
  assert.equal(result.data.requiresApproval, true);
...
test('collectionTills.inbound tipa maker/checker opt-in', async () => {
  const client = new Cimbra({ baseUrl: 'https://api.test', apiKey: 'cim_test', fetch: async () =>
    Response.json({
      ok: true, requiresApproval: true, replayed: false, deduplicated: false,
      approval: { id: 'approval_till_1', actionType: 'collection.till_credit', resourceType: 'collection_till', resourceId: 'till_1', status: 'pending' },
    }, { status: 202 }) });
  const result = await client.collectionTills.inbound('till_1', {
    externalReference: 'TILL-IN-1', description: 'Inbound MC', amount: '10.00', currency: 'ARS',
  });
  assert.equal(result.data.requiresApproval, true);
...
test('el SDK orquesta KYC/KYB por S2S y excluye la decisión humana', async () => {
  const calls: Array<{ url: string; method: string; idempotencyKey: string | null; body: Record<string, unknown> | FormData | null }> = [];
  const client = new Cimbra({ apiKey: 'cim_sk_test_example', baseUrl: 'https://api.test', maxRetries: 0, fetch: async (input, init) => {
    const url = String(input); const method = init?.method ?? 'GET';
    const rawBody = init?.body;
    const body = typeof rawBody === 'string' ? JSON.parse(rawBody) as Record<string, unknown>
      : rawBody instanceof FormData ? rawBody : null;
    ...
    if (url.endsWith('/compliance/documents')) return Response.json({ ok: true, document: { id: 'doc_1', fileName: 'ownership.pdf', status: 'received' } }, { status: 201 });
  ...
  await client.dueDiligence.uploadDocument({ file: '%PDF-1.4 evidence', fileName: 'ownership.pdf', contentType: 'application/pdf' });
  ...
  assert.deepEqual(calls.map(({ method, url }) => `${method} ${url}`), [
    ...
    'POST https://api.test/api/v1/compliance/documents',
  ...
  assert.deepEqual(calls[5].body, { checkType: 'sanctions', source: 'official_registry', status: 'passed', resultCode: 'no_match', note: 'Consulta directa.' });
