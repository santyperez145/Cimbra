  ...
  private async request<T>(method: Method, path: string, body?: unknown, options: RequestOptions = {}): Promise<CimbraResult<T>> {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt += 1) {
      ...
      try {
        ...
        if (!retryable || !shouldRetryStatus(response.status) || attempt === this.maxRetries) throw error;
        lastError = error;
      } catch (error) {
        if (error instanceof CimbraApiError) {
          if (!retryable || !shouldRetryStatus(error.status) || attempt === this.maxRetries) throw error;
          lastError = error;
        } else {
